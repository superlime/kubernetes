...
func testRollingUpdateDeployment(f *framework.Framework) {
	...
	rs := newRS(rsName, replicas, rsPodLabels, NginxImageName, NginxImage, nil)
...
func testDeploymentCleanUpPolicy(f *framework.Framework) {
	...
	_, err := c.AppsV1().ReplicaSets(ns).Create(newRS(rsName, replicas, rsPodLabels, NginxImageName, NginxImage, nil))
	framework.ExpectNoError(err)
...
func testRolloverDeployment(f *framework.Framework) {
	...
	rsReplicas := int32(1)
	_, err := c.AppsV1().ReplicaSets(ns).Create(newRS(rsName, rsReplicas, rsPodLabels, NginxImageName, NginxImage, nil))
	framework.ExpectNoError(err)
	// Verify that the required pods have come up.
