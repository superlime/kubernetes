...
getBaseImage() {
  os_arch=$1
  echo $(grep "${os_arch}=" BASEIMAGE | cut -d= -f2)
}
...
build() {
  ...
  for os_arch in ${os_archs}; do
    ...
    if [[ "$os_name" = "linux" ]]; then
      ...
      docker --tlsverify -H "${REMOTE_DOCKER_URL}" build --isolation=hyperv --pull -t "${REGISTRY}/${IMAGE_NAME}:${TAG}-${os_name}-${arch}-${os_version}" -f $dockerfile_name .
    else
